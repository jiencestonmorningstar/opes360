...
| 23 | E-signature | **B** | **Built 2026-08-16, inside Documents as the brief specified.** Sequential and parallel rounds, per-signer status, decline with reason, a public unauthenticated signing link resolved cross-tenant the same way `/v/{token}` verification already is. On completion, mints a `VerificationToken` the same way `DocumentIssuer` does on issue — no second verification system. **Completed 2026-09-18:** requesting a round now has a screen (the Show side panel — add signers, pick sequential or parallel, see per-signer status; a second round is refused while one is pending), and signers are actually told. Before that, a round was created and nobody was notified: a signer had to somehow already know their own signing link, so the feature only worked if a human copied the URL out and mailed it. Everyone who can act is now mailed on request, the next signer is told when a sequential turn arrives, and `opes:remind-pending-signatures` re-sends nightly at 07:45 with `last_reminded_at` so nobody is chased twice in a day. **Completed 2026-09-19:** a signature can name a block in the body and renders immediately after it, reusing the same `data-block-id` the editor already stamps for comment anchoring rather than inventing a second positioning scheme. Injection is on the rendered HTML, never on the stored body — a signature is not authored content, and writing it into the body would fold it into the tamper hash and make signing look like an edit to the document being signed. A deleted anchor loses the placement, not the signature. §19 is complete. |
