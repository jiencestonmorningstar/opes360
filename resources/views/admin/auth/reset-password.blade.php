...
<head>
    ...
    <title>Choose a new password · Platform admin · {{ config('opes.brand.name') }}</title>
...
<div class="w-full max-w-[380px]">
    ...
    <div class="card mt-7 p-6">
        <h1 class="text-[19px] font-bold tracking-[-0.02em] text-ink">Choose a new password</h1>
        ...
        <form method="POST" action="{{ route('admin.password.update') }}" class="mt-6 space-y-4">
            ...
            <div>
                <label for="password" class="block text-[13.5px] font-semibold text-ink-2">New password</label>
                <input id="password" name="password" type="password" required autocomplete="new-password"
                       class="mt-1.5 h-12 w-full rounded-xl border border-border bg-surface px-4 text-[15px] text-ink focus:border-brand focus:outline-none focus:ring-2 focus:ring-brand/20">
            </div>
            <div>
                <label for="password_confirmation" class="block text-[13.5px] font-semibold text-ink-2">Confirm new password</label>
